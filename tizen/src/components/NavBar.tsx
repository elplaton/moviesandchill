  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-14 py-3 flex items-center gap-6 transition-all duration-500 ${
      ...
      <div className="hidden md:flex items-center gap-1">
        <FocusableLink to="/" className={`px-3 py-1 text-sm rounded-lg transition-all duration-200 ${
          location.pathname === '/' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-200'
        }`}>Inicio</FocusableLink>
        <FocusableLink to="/movies" className={`px-3 py-1 text-sm rounded-lg transition-all duration-200 ${
          location.pathname === '/movies' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-200'
        }`}>Películas</FocusableLink>
        <FocusableLink to="/series" className={`px-3 py-1 text-sm rounded-lg transition-all duration-200 ${
          location.pathname === '/series' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-200'
        }`}>Series</FocusableLink>
        {isAdmin && (
          <FocusableLink to="/channels" className={`px-3 py-1 text-sm rounded-lg transition-all duration-200 ${
            location.pathname === '/channels' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-200'
          }`}>Canales</FocusableLink>
        )}
        <FocusableLink to="/settings" className={`px-3 py-1 text-sm rounded-lg transition-all duration-200 ${
