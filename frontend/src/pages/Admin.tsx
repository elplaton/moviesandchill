  ...
  return (
    <Shell>
      <div className="px-6 md:px-14 pt-24 pb-20 max-w-5xl mx-auto">
        ...
        <p className="text-nf-dim text-sm mb-8 animate-fade-in">Cuentas, cuotas, canales y configuración del servidor</p>

        <div className="flex gap-1 mb-8 border-b border-white/10">
          {TABS.map(t => (
            <button key={t.id} onClick={() => navigate(`/admin/${t.id}`)}
              className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === t.id ? 'text-white border-nf-red' : 'text-nf-dim border-transparent hover:text-nf-text'}`}>
              {t.label}
            </button>
          ))}
        </div>
